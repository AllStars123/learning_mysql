...
create table albums (
    ...
    index album_name_idx(album_name)
);

drop table if exists paid_subscription;
create table paid_subscription (
    start_date datetime,
    end_date datetime,
    user_id BIGINT UNSIGNED NOT NULL,
    track_lis_id BIGINT UNSIGNED NOT NULL,

    foreign key (user_id) references users(id),
    foreign key (track_lis_id) references track_list(id)

);

select 'programma' like 'progr%'
